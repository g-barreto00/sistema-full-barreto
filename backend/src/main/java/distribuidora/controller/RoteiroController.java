...
public class RoteiroController {
    ...
    @PatchMapping("/{id}/finalizar")
        public ResponseEntity<?> finalizar(@PathVariable Long id,
            ...
            List<Integer> entreguesInt = body.getOrDefault("pedidosEntregues", List.of());
            List<Long> entregues = entreguesInt.stream().map(Integer::longValue).toList();

            for (Long pedidoId : entregues) {
            ...
            pedidoService.listarTodos().stream()
                .filter(p -> p.getRoteiro() != null &&
                            ...
                            !entregues.contains(p.getNumeroPedido()))
